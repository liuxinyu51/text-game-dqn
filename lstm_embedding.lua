...
return function(args)
    ...
    function create_network(args)
        ...
        -- Multiple Linear (action decision learning)
        lstm_seq:add(nn.Linear(n_hid, n_hid*4))
        lstm_seq:add(nn.Rectifier())
        lstm_seq:add(nn.Linear(n_hid*4, n_hid*8))
        lstm_seq:add(nn.Rectifier())
        lstm_seq:add(nn.Linear(n_hid*8, n_hid))
        lstm_seq:add(nn.Rectifier())
